...
export function FeaturedArticle({ article }: { article: Article }) {
  ...
  return (
    <a
      href={`/articles/${slug}`}
      className="group border-border-subtle bg-surface-elevated hover:border-border-strong flex flex-col gap-3 rounded-xl border p-12 shadow-[0_1px_3px_var(--shadow-1a)] transition-colors"
    >
      <span className="text-text-accent font-mono text-xs tracking-[1px] uppercase">
        Featured essay · {formatStamp(date)}
      </span>
      <h2 className="text-text-primary group-hover:text-link max-w-[900px] text-[32px] leading-[1.2] font-semibold tracking-[-0.5px] transition-colors">
        {title}
      </h2>
